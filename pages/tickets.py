...
def render_tickets_page():
    ...
    with col_inspector:
        ...
        # Single Unified Inspector Container
        with st.container(border=True):
            ...
            st.markdown("<div style='font-size: 0.72rem; font-weight: 700; color: #94a3b8; letter-spacing: 0.05em; text-transform: uppercase; margin-bottom: 4px;'>Customer Inquiry</div>", unsafe_allow_html=True)
            st.markdown(
                f"""<div style="background-color: rgba(255, 255, 255, 0.025); border: 1px solid rgba(255, 255, 255, 0.07); border-radius: 6px; padding: 10px 12px; font-size: 0.88rem; line-height: 1.5; color: #e2e8f0; white-space: pre-wrap;">{ticket['description']}</div>""",
                unsafe_allow_html=True
            )
            ...
            if analysis:
                ...
                with m2:
                    st.markdown(f"<div style='font-size: 0.7rem; color: #94a3b8;'>DEPARTMENT</div><div style='font-size: 0.85rem; font-weight: 600; color: #f1f5f9;'>{analysis['department']}</div>", unsafe_allow_html=True)
                with m3:
                    st.markdown(f"<div style='font-size: 0.7rem; color: #94a3b8;'>PRIORITY</div><div style='margin-top: 2px;'>{get_priority_badge(analysis['priority'])}</div>", unsafe_allow_html=True)
                with m4:
            ...
            if suggested_res:
                ...
                if sources:
                    chips_html = " ".join([
                        f'<span style="background-color: rgba(59, 130, 246, 0.1); border: 1px solid rgba(59, 130, 246, 0.25); border-radius: 4px; padding: 3px 8px; font-size: 0.78rem; font-weight: 500; color: #93c5fd; display: inline-block; margin: 2px 4px 4px 0;">{s}</span>'
                        for s in sources
                    ])
